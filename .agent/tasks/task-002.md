...
files_affected:
  ...
  - .agent/tasks/         → agent/routing/tasks/
...
`.agent/tasks/` contains ACP Enhanced routing tasks (`task-001.md`, `task-002.md`).
...
agent/
  ...
  routing/           ← ACP Enhanced: taxonomy, rules, ledger, config
    tasks/           ← ACP Enhanced: routing task files (was .agent/tasks/)
...
git mv .agent/tasks/task-template.md agent/routing/tasks/task-template.md
git mv .agent/tasks/task-001.md      agent/routing/tasks/task-001.md
git mv .agent/tasks/task-002.md      agent/routing/tasks/task-002.md
...
for f in "${FILES[@]}"; do
  # Replace .agent/tasks/ → agent/routing/tasks/ FIRST (more specific pattern)
  sed -i '' 's|\.agent/tasks/|agent/routing/tasks/|g' "$f"
...
`acp-dispatch.ts` currently expects tasks at `.agent/tasks/task-NNN.md`.
