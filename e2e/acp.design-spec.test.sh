...
# ═══════════════════════════════════════════════════════════════════════════════
# Assertion 11: No FIFOZ hardcoded paths in command doc
# ═══════════════════════════════════════════════════════════════════════════════
print_test_header "acp.design-spec.md: no hardcoded FIFOZ paths"
# The command should not contain literal FIFOZ project paths as the ONLY option
# (it may mention them in stack detection examples)
fifoz_paths=$(grep -c "frontend/store/\|backend/server.py" "$CMD_FILE" 2>/dev/null || echo 0)
if [[ "$fifoz_paths" -le 2 ]]; then
  assert_true "FIFOZ paths are examples only (found $fifoz_paths refs)" 0
else
  assert_true "FIFOZ paths are examples only (found $fifoz_paths refs)" 1
fi
