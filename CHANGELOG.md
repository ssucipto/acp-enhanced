...
- **Gate fixture** — `tests/fixtures/coderabbit-findings-sample.json` (sanitized AE `--agent` NDJSON + consumer-project inventory rows).
- **`acp.findings-import.sh`** — `--input` / `--dry-run` → `audit-carryovers.md` when `coderabbit_active` (no `--pr` in v1).
- **`.coderabbit.yaml` starter** — `agent/templates/coderabbit.yaml.template` + wiki bootstrap.
- **`/acp-review` augmentation** — Phase 1 never deferred; Phase 2 annotate + import pointer (weekly command unchanged, F-101-02).
- **E2E** — `e2e/coderabbit-integration.test.sh` matrix A–D.

### Notes
- Target version historically listed as v6.29.0/v6.30.0; those tags were taken by M83/other work — M81 ships as **v6.32.0**.
- Aikido / M76 / M77 remain ADR-19 gated.
...
### Added (M86 — consumer-project field-feedback port)
...
- **Fork upgrade checklist** — `docs/acp-fork-upgrade-checklist.md` + consumer-project upgrade note correcting the six-unreported false positive (ADR-26).
- **Rule-verification discipline pattern** — proxy ≠ invariant (feedback-008).

### Changed
- **review-scan** — diff-merge consumer-project feedback-008 precision into AE M83 (no blind overwrite); see `m86-review-scan-merge-notes.md`.
...
- **`/acp-stakeholder-report`** — Generate concise weekly/monthly stakeholder progress summaries with RAG health indicator, ≤300-word executive summary, decisions required, and 2–4 KPI metrics. v1.1.0 hardened by audit-071 from consumer-project production use.
...
### Added
- `agent/reports/audit-008-feedback-001-knowledge-preservation.md` — audit report on consumer-project knowledge loss incident (3 sessions lost to context overflow); 6 findings, 5 decisions (R1–R4 adopted, R5 deferred)
...
- `agent/memory/lessons.md` — prepended high-priority `acp-knowledge-gap` postmortem (2026-05-09, task_type: all); documents 3-session consumer-project loss, 5 correction points
