...
5b. **F-091-14 addability check**: gitignore check must probe *new-file addability* in protocol directories (`git check-ignore` on a probe path under agent/reports/, agent/memory/, agent/tasks/) — ERROR if a protocol dir rejects new files; also ERROR if files on disk in agent/reports/ are untracked
