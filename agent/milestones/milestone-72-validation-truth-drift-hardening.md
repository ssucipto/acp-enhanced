...
Close **all audit-091 findings (F-091-01..13)** and **all addressable pending carryovers** (CRIT-065-002; F-086-02 stays on task-239/M71 ops). Core principle from the design doc: *a drift fix only counts when it lands together with the enforcement check that would have caught it.* No feature work — hardening only.

## Build Order

| Phase | Route | Task | Title | Findings | Est. | Priority |
|-------|-------|------|-------|----------|------|----------|
| **1** | route-229 | task-240 | Drift closures (versions, sync, registrations, reports gitignore) | F-091-01, 02, 14 (fix half) | 2h | P0 |
| **1** | route-230 | task-241 | Validator hardening (root, zero-fail, hash, pkg, 5-surface, addability) | F-091-01..05, 14 (enforce half) | 5h | P0 |
| **2** | route-231 | task-242 | Wrapper strays + bootstrap dead-glob cleanup | F-091-04, F-091-06 | 2h | P1 |
...
| **2** | route-233 | task-244 | ShellCheck CI gate | F-091-10 | 3h | P1 |
...
| **4** | route-236 | task-247 | M72 closure audit (audit-092) + ship v6.27.0 | all F-091 re-verify | 3h | P0 |
...
| [task-247](../tasks/milestone-72-validation-truth-drift-hardening/task-247-m72-closure-ship.md) | route-236 | audit-092 PASS; v6.27.0 tagged; carryovers honestly stamped | M72 complete |
...
3. No carryover stamped `fixed` without audit-092 re-verification.
...
- [ ] audit-092 PASS verdict; all F-091 carryovers `fixed` with audit-092 pointer
