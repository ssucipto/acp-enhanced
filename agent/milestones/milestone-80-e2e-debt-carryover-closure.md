...
**Status**: planned (0/4)
**Estimated effort**: ~7h (4 tasks)
**Source**: audit-099 (F-M78-01 root-cause triage) + F-086-02
**Depends on**: M79 (v6.28.1)

## Goal

Close the two remaining open carryovers honestly. F-M78-01 is the 7 genuinely pre-existing E2E test failures root-caused in audit-099 — a mix of stale test expectations, a fixture referencing a nonexistent file, and script-vs-test behavior mismatches. F-086-02 is the FIFOZ downstream `/acp-version-update` verification, which is blocked on consumer-repo access. This milestone fixes what is fixable, decides code-vs-test per case (no blind greening), and documents anything irreducible.
...
| route-256 | task-267 | F-086-02 FIFOZ downstream verification (ops) | blocked precondition: consumer repo | 1h |
| route-257 | task-268 | M80 closure — full suite, v6.28.2, close carryovers | closure | 1h |

Dependency: task-268 depends on 265–267. task-267 is ops-gated (may stay blocked).
...
4. **Honest F-086-02** — do NOT mark it fixed without actual downstream verification; if consumer-repo access is unavailable, keep it deferred with the precise precondition.
5. **Document irreducible** — if any failure reflects an intentional design choice the test wrongly encodes, record the decision rather than forcing green.
...
- [ ] F-086-02: verified downstream OR kept pending with precondition (task-267)
