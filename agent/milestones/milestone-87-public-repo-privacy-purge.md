...
**Status**: planned  
**Progress**: 0/11 tasks  
**Estimated effort**: ~28h (11 tasks, 5 phases)  
...
| ID | Name | Est | Depends |
|----|------|-----|---------|
| 322 | ADR-27 already in-tree; lock design + command citation map | 2h | — |
| 323 | Local encrypted archive + restore test | 3h | 322 |
| 324 | Gitignore + reverse D9 validator | 3h | 322 |
| 325 | Commands/E2E/wiki: reports are local | 3h | 324 |
| 326 | Pattern + project-create + bootstrap gitignore | 2h | 324 |
| 327 | Redact `$HOME` and consumer internals in remaining tracked files | 3h | 322 |
| 328 | `git rm` report/feedback bodies; keep `.gitkeep`+README | 2h | 323, 324, 327 |
| 329 | `acp.private-pack.sh` + docs | 3h | 323 |
| 330 | `git filter-repo` SOP; operator-confirmed force-push develop+mainline | 4h | 323, 328 |
| 331 | Fresh-clone verification (no bodies in `git log --all`) | 2h | 330 |
| 332 | Closure: validate, stamp F-118, CHANGELOG v6.33.0 | 3h | 325, 326, 329, 331 |

## Anti-shortcuts

- F-118-01..03 stay **pending** until task-331 clone proof.
- No `/acp-proceed` force-push without the operator’s explicit “force-push yes”.
- Do not add new `agent/reports/audit-*.md` to origin during this milestone.
