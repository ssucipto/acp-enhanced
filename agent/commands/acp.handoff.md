...
**Executor mode** encodes what FIFOZ field evidence (M51 exemplar) proved works: ADR locks, task sequences, git pins, scope guardrails, and a return-handoff path — all mandatory in the template below.
...
| `--scope <slug>` | | Executor | Scope slug for filename (e.g. `m51-fifoz-pro`). If omitted, derive from milestone/task/conversation and kebab-case |
...
| Implementation steps | Allowed in executor only | FIFOZ M51 evidence: receiving agent needs task sequence, not replanning |
